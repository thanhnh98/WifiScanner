...
internal fun copyToCache(context: Context, imageUri: Uri, imageFile: File): File {
    val desFile = File("${cachePath(context)}${imageFile.name}")
    return imageFile.copyToCache(context, imageUri, desFile, true)
}
...
fun String.getFileName(): String{
    ...
    if (list.isNotEmpty() && list[list.size - 1].length > 1){
        return list[list.size - 1].apply {
...
fun File.copyToCache(context: Context, imageUri: Uri, target: File, overwrite: Boolean = false, bufferSize: Int = DEFAULT_BUFFER_SIZE): File {
    if (!this.exists()) {
