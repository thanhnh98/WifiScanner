...
inline fun <reified VM : ViewModel, T> T.kodeinViewModelFromParentFragment(): Lazy<VM> where T : KodeinAware, T : Fragment {
    return lazy { ViewModelProvider(this.requireParentFragment(), direct.instance()).get(VM::class.java) }
}